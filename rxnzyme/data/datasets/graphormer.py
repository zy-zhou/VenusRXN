...
from .base import ignore_label, Batch, LMDBDataset
from ..reaction.graph import x_vocabs, one_hots_to_labels
from ..reaction.collator import pad_2d, collate
from ..reaction.database import decompress_mol_graph

def is_partial_rxns(rxn_db_dir):
    cols = pd.read_csv(
        os.path.join(rxn_db_dir, 'metadata.csv'), nrows=0
    ).columns
    return 'center_size' not in cols
...
class RxnDataset(LMDBDataset):
    def __init__(
        ...
        self.is_partial = is_partial_rxns(db_dir)
