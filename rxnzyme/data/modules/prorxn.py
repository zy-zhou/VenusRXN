...
from ..datasets.graphormer import RxnDataset
from ..datasets.prorxn import (
    EnzymeDataset,
    RxnzymeDataset,
    RxnzymeDatasetForLTR
)
from ..samplers.prorxn import (
    RxnzymeSamplerForCL,
    DistributedRxnzymeSamplerForCL,
    RxnzymeSamplerForBLTR,
    DistributedRxnzymeSamplerForBLTR
)
...
class ProRxnDataModuleForLTR(LightningDataModule):
    ...
    def setup(self, stage='fit'):
        ...
        if stage in {'fit', 'validate'}:
            ...
            self.pred_dataset = RxnzymeDataset(
                rxn_db_dir=self.rxn_db_dir,
                enz_db_path=self.enz_db_path,
                rxn_enz_ids=self.pred_ids,
                tokenizer=self.tokenizer,
                spatial_pos_max=self.train_config['multi_hop_max_dist'],
                mask_unreachable=False,
                max_length=self.train_config['max_length']
    ...
    def train_dataloader(self):
        if self.train_config['loss_fn'] in {'bce', 'brank'}:
            if self.trainer.world_size > 1:
                batch_sampler = DistributedRxnzymeSamplerForBLTR(
                    self.train_dataset,
                    batch_size=self.train_config['train_batch_size'],
                    rank=self.trainer.global_rank,
                    world_size=self.trainer.world_size,
                ...
                batch_sampler = RxnzymeSamplerForBLTR(
                    self.train_dataset,
                    batch_size=self.train_config['train_batch_size'],
                    shuffle=True
                )
